...
export const siteConfig = {
  ...
  pipeline: {
    ...
    voice: {
      ...
      /** Composition rules — substantive, self-contained article body. */
      compositionRules:
        "ABSOLUTE REQUIREMENT — The body must be FULLY READABLE WITHOUT CLICKING THE SOURCE. A reader who never visits the source link should finish your article understanding (a) WHAT the tool / news is, (b) WHO ships it / WHO it's for, (c) WHEN it launched, (d) WHY it matters technically and commercially, (e) HOW it actually works (model, architecture, pricing tier, integration surface), and (f) WHAT to watch next. The source link exists for verification and deep dives, never to fill in basic gaps the article leaves open. If you write 'the company added new features' without naming the features, STOP — that sentence belongs in a vendor press release, not in AITECH TOKYO.\n\n- Length: 5–8 substantial paragraphs in each language. Target ~450–650 words in English, ~900–1300 full-width characters in Japanese. Short paragraphs (2–3 sentences) are fine for rhythm, but the TOTAL information density per article must be high.\n\n- STRUCTURE (recommended, not rigid):\n  · Paragraph 1 (opening): State WHAT the tool / news is in one declarative sentence, then name WHO ships it (specific company, lab, founder) and WHEN. Do NOT paraphrase the headline. Get into the substance immediately.\n  · Paragraph 2: WHY this matters — the mechanism, the model used (GPT-4o / Claude 3.5 / Llama 3 / open-weight), the technical primitive, the pricing signal. Cite concrete numbers, model names, vendor names, infra integrations if they appear in the source.\n  · Optional '## subheading' (e.g. '## What it actually is' / '## How the model works'): the HOW. The exact architecture, API surface, pricing tier, integration mode. After reading this section, a senior developer should be able to brief their team on what's shipping and how it differs from the existing stack.\n  · Middle paragraphs: CONTEXT — who else is doing this (Anthropic, OpenAI, Google, Mistral, Meta, Cohere, Cursor, GitHub Copilot, Codeium, etc.), how it compares to incumbents, what specific named competitors appear in the source.\n  · Optional '## What comes next' subheading: the WHAT'S NEXT — broader implications for the AI / dev-tool stack, what to watch in the coming weeks or quarter, the open questions.\n  · Optional ONE '> pull-quote' line from the original dispatch, placed near the relevant paragraph.\n\n- Name concrete details: model used, pricing tier, host country, what infra it integrates with (Vercel / Cloudflare / AWS / Bedrock / Vertex / Azure), VC if disclosed, geo, KPI / outcome.\n- Do NOT fabricate funding amounts, founder names, model names, dates, or pricing. If a fact is not in the source, omit it. Better quiet than wrong.\n- Do NOT repeat the article title verbatim as the first body paragraph.\n- PRESERVE EMBEDDED MEDIA — When the source article references specific external resources (YouTube videos, vendor demo pages, official launch pages, GitHub repos, model cards, official documentation, technical blog posts, paper PDFs, Hugging Face spaces, Product Hunt pages), INCLUDE THOSE URLs INLINE in the body using markdown link syntax: [descriptive label](URL). The reader experience must be: AITECH TOKYO's article + the inline media links = the COMPLETE package. The source link at the bottom of the page is for verification of facts only — it should never be the place where the reader has to go to FIND the referenced demo video, model card, GitHub repo, or official documentation. Examples of acceptable inline links:\n  · [Watch the demo on YouTube](https://youtube.com/watch?v=...)\n  · [Model card on Hugging Face](https://huggingface.co/...)\n  · [Official launch post](https://www.vendor.com/blog/...)\n  · [GitHub repo](https://github.com/org/repo)\n  · [Documentation](https://docs.vendor.com)\n  Do NOT invent URLs — only include links that actually appear in the source dispatch. If the source dispatch references a video but doesn't link to it, omit the link.\n\n- The 4 STRUCTURED FIELDS (tagline / useCase / vsCompetitor / tokyoTake) and the long-form TOKYO TAKE block are emitted IN ADDITION to a fully-fleshed body — they are value layered on top of a complete, self-sufficient article, NEVER a replacement for the substantive explanation the reader needs to understand the story.",
      /** Japanese-specific rules — self-contained Japanese article, not translation. */
      japaneseRules:
        "ABSOLUTE REQUIREMENT — 日本語版は独立した記事である。英語の原文を一切参照しなくても、東京のビジネスパーソン／開発者が「何のツール／ニュースか／誰がやったか／いつか／どこが技術的に新しいか／既存のスタックとどう違うか／次に何を見るか」を全て理解できる状態で書く。原文への外部リンクは検証用であって、本文の補足ではない。\n\n- 単なる英語の翻訳ではなく、日本のテック・ビジネス読者向けに書き直された PARALLEL ARTICLE。語彙、リズム、見出しの切り口は日本語編集者が自分で書いたものとして自然であること。\n\n- 文量：本文は 5〜8 段落、合計でおおむね 900〜1300 文字。短い段落（2〜3 文）でリズムを作るのは可だが、1 記事あたりの情報密度は高く保つ。\n\n- 専門用語の取扱い：海外発の専門用語（context window、RAG、agentic、tool use、MCP、function calling、fine-tuning、distillation、quantization、inference cost、token cost、Privacy Sandbox、on-device、edge inference 等）は初出で簡潔に意味を添える。例：「RAG（外部知識を取り込んで回答精度を上げる検索拡張生成）」「MCP（Anthropic が公開した、外部ツールと LLM を繋ぐ標準プロトコル）」。\n\n- 読者が段落 2 の終わりまでに、「結局これは何のツールで、何の問題を解いているのか？」を完全に理解できる状態にする。曖昧に締めない。\n\n- 常体（だ・である調）を基本に、クリーンな現代日本語で書く。漢字と平仮名を自然に混ぜる。固有名詞（GPT-4、Claude、Cursor、Anthropic、OpenAI、Llama、Gemini、Mistral、Cohere、Codeium、Product Hunt、Hacker News、Bedrock、Vertex、Cloudflare、Vercel）はローマ字／略語表記を優先。「エルエルエム」「エーピーアイ・キー」のような不自然なカタカナ展開は避け、「LLM」「API キー」と書く。\n\n- 記号：引用句は「」、編集者のアサイドは — (em-dash)。\n\n- 日本企業・サービスは標準的な表記：ELYZA、Sakana AI、Preferred Networks、PLAUD、Notta、LINEヤフー、freee、Money Forward、サイバーエージェント、Mercari、Cookpad 等。\n\n- 禁句リスト：「革新的」「次世代」「ゲームチェンジャー」「破壊的」「DX で変わる」「顧客体験を再定義」「シームレスな」「次世代 AI」「魔法のような」.\n\n- 見出し（## subheading）と pull-quote は宣言的に。「何が出たのか」「どこが新しいのか」「日本での使いどころ」「次に何を見るか」など、読者に内容を即座に伝える短い切り口。",
